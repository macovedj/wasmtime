...
// Winch can't keep GC references alive across a collection, so every
// position that would put an externref in a frame has to be rejected at
// compile time.
#[wasmtime_test(strategies(only(Winch)), wasm_features(reference_types))]
#[cfg_attr(miri, ignore)]
fn externref_values_are_unsupported(config: &mut Config) -> Result<()> {
    let engine = Engine::new(config)?;
    let cases = [
        ("local", r#"(module (func (local externref)))"#),
        (
            "param",
            r#"(module (func (param externref) (drop (local.get 0))))"#,
        ),
        (
            "call result",
            r#"(module
                 (import "" "f" (func $f (result externref)))
                 (func (result externref) (call $f)))"#,
        ),
        (
            "dropped call result",
            r#"(module
        ...
        (
            ...
            r#"(module
                 ...
                 (func (param externref) (call $f (local.get 0))))"#,
        ),
        (
            "global.get",
            r#"(module
                 (global $g externref (ref.null extern))
                 (func (drop (global.get $g))))"#,
        ),
        (
            "table.get",
            r#"(module
                 (table $t 1 externref)
                 (func (drop (table.get $t (i32.const 0)))))"#,
        ),
        (
            "ref.null extern",
            r#"(module (func (drop (ref.null extern))))"#,
        ),
        (
            "block signature",
            r#"(module (func (block (result externref) (ref.null extern)) (drop)))"#,
        ),
    ];
    for (what, wat) in cases {
        let err = Module::new(&engine, wat)
            .err()
            .unwrap_or_else(|| panic!("{what}: expected an error, but the module compiled"));
        assert!(
            format!("{err:?}").contains("Unsupported Wasm type"),
