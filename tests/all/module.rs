...
mod native_unwind {
    ...
    fn native_unwind_sections_survive_non_lifo_removal() -> Result<()> {
        // Run alone: another test could otherwise reuse a dropped module's code
        // addresses before we check that its unwind entries were removed.
        const CHILD: &str = "WASMTIME_TEST_UNWIND_REGISTRY_CHILD";
        if std::env::var_os(CHILD).is_none() {
            let name = concat!(
                module_path!(),
                "::native_unwind_sections_survive_non_lifo_removal"
            );
