...
- nested component lowering through canonical ABI trampolines;
...
### Benefit-oriented examples

Tail calls provide bounded stack use even where they do not improve execution
time. With Winch's maximum Wasm stack set to 64 KiB, an ordinary recursive
countdown traps with `StackOverflow` before completing one million calls. The
equivalent `return_call` countdown completes all one million calls and returns
the expected value. This is covered by a regression test rather than a timing
benchmark because the important difference is capability, not latency.

An additional benchmark compares two equivalent alternating state machines:
a bounded-stack dispatch loop that makes ordinary calls to state handlers, and
mutually recursive state handlers connected by tail calls. Both fixtures are
checked to produce 16 after four transitions. The AArch64 measurements below
are microseconds per 100,000 transitions from two consecutive runs collected
on 2026-08-26. Both runs are shown because the Winch result moved enough that a
single percentage would be misleading.

| Compiler | Run | Dispatch loop | Mutual tail calls | Tail / dispatch |
| --- | ---: | ---: | ---: | ---: |
| Winch | 1 | 247.18 | 256.39 | 1.037x |
| Winch | 2 | 245.95 | 245.43 | 0.998x |
| Cranelift | 1 | 113.90 | 49.99 | 0.439x |
| Cranelift | 2 | 111.96 | 48.79 | 0.436x |

This example does not currently demonstrate a repeatable Winch speedup: its
tail-call version ranges from parity with the bounded dispatcher to 3.7%
slower. Cranelift consistently reduces runtime by about 56%, showing that the
tail-recursive shape can remove substantial dispatcher overhead after
optimization. For Winch, the comparison identifies a concrete remaining
optimization opportunity while the bounded-stack regression demonstrates the
immediate semantic benefit.
