    ...
    M: MacroAssembler,
{
    ...
    pub(crate) fn emit_null_gc_alloc(
        ...
        self.masm.load(
            self.masm.address_at_reg(heap_data, 0)?,
            writable!(bump),
            OperandSize::S32,
        )?;
        ...
        self.masm.store(
            shared_ty.into(),
            self.masm
                .address_at_reg(object_addr, VM_GC_HEADER_TYPE_INDEX_OFFSET)?,
            OperandSize::from_bytes(shared_ty_size),
        )?;
        self.context.free_reg(shared_ty);
        let heap_data = self.context.any_gpr(self.masm)?;
        self.masm.load_ptr(
            self.masm.address_at_vmctx(u32::from(heap_data_offset))?,
            writable!(heap_data),
        ...
        let end = self.context.any_gpr(self.masm)?;
        self.masm
            .mov(writable!(end), aligned.reg.into(), OperandSize::S32)?;
        self.masm.add(
            writable!(end),
            end,
            RegImm::i32(size.cast_signed()),
