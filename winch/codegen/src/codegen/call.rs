...
impl FnCall {
    ...
    pub fn emit<M: MacroAssembler>(
        ...
        // Record a stack map for this safepoint. The spill above parked
        // every live value in a frame slot, so GC refs on the value stack
        // are enumerable from compile-time state alone.
        ...
        let mut map_offsets: Vec<u32> = context
            ...
            .collect();
        // Ref-typed locals are also live roots. Stack arguments are
        // addressed from the frame pointer and live in the caller's frame,
        // outside this map's span.
        map_offsets.extend(
            context
                .frame
                .locals()
