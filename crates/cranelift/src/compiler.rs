    ...
    fn compile_function(
        ...
        if matches!(
            ...
            if state.has_candidates() {
                // Remove frontend-only dead blocks and constant block
                // parameters before asking the range analysis to follow value
                // flow through loops. The normal optimization pipeline
                // performs these same idempotent cleanups again later.
                context.flowgraph();
                context
